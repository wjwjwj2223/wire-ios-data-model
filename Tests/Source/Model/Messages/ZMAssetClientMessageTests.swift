...
class BaseZMAssetClientMessageTests : BaseZMClientMessageTests {
    ...
    func appendImageMessage(to conversation: ZMConversation, imageData: Data? = nil) -> ZMAssetClientMessage {
        ...
        let uploaded = WireProtos.Asset(otrKey: .randomEncryptionKey(), sha256: .zmRandomSHA256Key())
...
extension ZMAssetClientMessageTests {
    ...
    func testThatItUpdatesTheTransferStateWhenTheUploadedMessageIsMerged()
    {
        ...
        // when
         var originalMessage = GenericMessage(content: WireProtos.Asset(otrKey: .zmRandomSHA256Key(), sha256: .zmRandomSHA256Key()), nonce: nonce)
    ...
    func testThatItDoesntUpdateTheTransferStateWhenTheUploadedMessageIsMergedButDoesntContainAssetId()
    {
        ...
        // when
        let originalMessage = GenericMessage(content: WireProtos.Asset(otrKey: .zmRandomSHA256Key(), sha256: .zmRandomSHA256Key()), nonce: nonce)
        sut.update(with: originalMessage, updateEvent: ZMUpdateEvent(), initialUpdate: true)
    ...
    func testThatItUpdatesTheTransferStateWhenTheCanceledMessageIsMergedAfterUploadingSuccessfully()
    {
        ...
        let canceledMessage = GenericMessage(content: asset, nonce: nonce)
    ...
    func testThatItReturnsTheEncryptedUploadedDataWhenItHasAUploadedGenericMessageInTheDataSet() {
        self.syncMOC.performAndWait { 
            ...
            sut.add(GenericMessage(content: WireProtos.Asset(otrKey: otrKey, sha256: sha256), nonce: sut.nonce!))
            
            // then
            XCTAssertNotNil(sut)
            guard let asset = sut.underlyingMessage?.asset else { return XCTFail() }
            guard case .uploaded? = asset.status else { return XCTFail() }
    ...
    func testThatItDoesNotSetAssetIdWhenUpdatingFromAnUploadedMessage() {
        self.syncMOC.performAndWait {
            ...
            let assetWithUploaded = WireProtos.Asset(otrKey: Data.zmRandomSHA256Key(), sha256: Data.zmRandomSHA256Key())
            ...
            let asset = WireProtos.Asset.with({
                ...
                $0.preview = assetWithPreview.preview
            })
...
extension ZMAssetClientMessageTests {
    ...
    func testThatItCreatesOTRAssetMessagesFromAssetNotUploadedFailedUpdateEvent() {
        ...
        let dataPayload = [
            "info" : try? genericMessage.serializedData().base64String(),
            "id" : thumbnailId
            ]
...
extension ZMAssetClientMessageTests {
    ...
    private func originalGenericMessage(nonce: UUID, image: WireProtos.Asset.ImageMetaData? = nil, preview: WireProtos.Asset.Preview? = nil, mimeType: String = "image/jpg", name: String? = nil) -> GenericMessage {
        let original = WireProtos.Asset.Original.with {
            $0.size = 128
            $0.mimeType = mimeType
            $0.name = name ?? ""
        ...
        return GenericMessage(content: asset, nonce: nonce)
    }

    private func uploadedGenericMessage(nonce: UUID, otr: Data = .randomEncryptionKey(), sha: Data = .zmRandomSHA256Key(), assetId: UUID? = UUID.create(), token: UUID? = UUID.create()) -> GenericMessage {
        
        let remote = WireProtos.Asset.RemoteData.with {
            $0.otrKey = otr
            $0.sha256 = sha
            if let assetId = assetId {
                $0.assetID = assetId.transportString()
            }
            if let token = token {
                $0.assetToken = token.transportString()
            }
        }
    ...
    func testThatItReportsDownloadedFileWhenThereIsAnImageFileInTheCache_V3() {
        ...
        let assetId = UUID.create()
        let assetData = Data.secureRandomData(length: 512)
        let image = WireProtos.Asset.ImageMetaData.with {
            $0.width = 123
            $0.height = 4569
        }
